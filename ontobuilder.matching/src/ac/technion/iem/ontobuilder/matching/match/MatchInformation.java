...
public class MatchInformation
{
     ...
     * 
     * @return {@link JTable} holds target,candidate names, algorithm, total count of matches precision and
     * recall
     */
    ...
    public JTable getStatistics(SchemaTranslator exactMapping)
    {
        ...
        {
            recall = getRecall(exactMapping);
            precision = getPrecision(exactMapping);
        }
        Object columnNames[] =
        {
    ...
    {
        SchemaTranslator st = new SchemaTranslator(this, true, false);
        ...
        List<?> goodMatches = (exact != null ? SchemaMatchingsUtilities.intersectMappings(st,
            exact, getMatrix()) : new ArrayList<Object>());
        ...
        DefaultGraphCell targetCell = targetOntology.getModel().addToGraph(cells, attributes, cs);
        DefaultGraphCell candidateCell = candidateOntology.getModel().addToGraph(cells, attributes,
            cs);
        ...
        for (Iterator<?> i = allMatches.iterator(); i.hasNext();)
        {
            ...
            Term cand, target;
            cand = getMatrix().getTermByName(match.getAttribute2());
            target = getMatrix().getTermByName(match.getAttribute1());
    ...
    // public JGraph getGraph(SchemaTranslator exact)
    // {
    // SchemaTranslator st = new SchemaTranslator(this);
    // List goodMatches = SchemaMatchingsUtilities.diffBestMatches(st, exact);
    // JGraph graph=new JGraph(new DefaultGraphModel());
    // graph.setEditable(false);
    ...
    //
    // DefaultGraphCell targetCell=targetOntology.getModel().addToGraph(cells,attributes,cs);
    // DefaultGraphCell candidateCell=candidateOntology.getModel().addToGraph(cells,attributes,cs);
    //
    // NumberFormat nf = NumberFormat.getInstance();
    //
    // // Create match edges
    ...
    // DefaultGraphCell cell=(DefaultGraphCell)object;
    // Object userObject=cell.getUserObject();
    // if(userObject!=null && userObject.equals(targetTerm))
    // targetTermCell=cell;
    // else if(userObject!=null && userObject.equals(candidateTerm))
    // candidateTermCell=cell;
    // }
    // }
    //
    ...
    // DefaultPort toCandidatePort=new DefaultPort("toCandidate");
    // targetTermCell.add(toCandidatePort);
    // DefaultPort toTargetPort=new DefaultPort("toTarget");
    // candidateTermCell.add(toTargetPort);
